package com.github.gpor0.jaffas.generators;

import com.github.gpor0.jaffas.exceptions.UnknownManagedException;
...
public class IdGenerator {
    ...
    private String salt_ = "1wjL;A--BA!|H)lUMSD+.uM5z>Q4~V{_ v_O+F&kGSyENiAM1Gp%n^wr^{T-2aG+";
    ...
    public IdGenerator(int minHashLength) {
        this(minHashLength, DEFAULT_ALPHABET);
    }

    public IdGenerator(int minHashLength, String alphabet) {
    ...
    public static String getId() {
        return new IdGenerator(10).encrypt(
                new SecureRandom().nextInt(Integer.MAX_VALUE)).toUpperCase();
    }

    public static String getPrefixedTimedId(int length, String prefix, Function<String, Object> check) {
        int yr = LocalDateTime.now().getYear() % 100;
        String base = prefix + yr;
        String randomCode = new IdGenerator(10).encrypt(new SecureRandom().nextInt(Integer.MAX_VALUE)).substring(0, length - base.length());
        String code = (base + randomCode).toUpperCase();

        if (check != null) {
            int i = 0;
            while (check.apply(code) != null) {
                if (i > 15) {
                    //infinite loop violation check
                    //looks like there is a problem in random code generation: either too long prefix and to short length resulting in low entropy causing unable to find free codes
                    throw new UnknownManagedException(new Exception("errors.noCodesLeft"));
                }
                code = getPrefixedTimedId(length, prefix, null);
                i++;
            }
        }
